(async () => {
        ...
        for (const key of keys) { // loop through cart details
            ...
            new Promise(async (resolve, reject) => {
                try {
                    ...
                    let stock = user_data.updated ? await pullAccountCurrentStock(user_data.ver, user_data.name) : await checkCurrentStock(user_data.ver, user_data.name); // supabase vs firebase
                    ...
                    clone.id = key;
                    clone.dataset.accname = `${user_data.name}`;
                    clone.dataset.accver = `${user_data.ver}`;
                    queryP(clone, 'cart-item-name').textContent = `${user_data.name} (${user_data.ver})`;
                    ...
                    if (isAccountReady() && stock > 0) {
                        ...
                        // For the mean time unlimited time!
                        queryP(clone, 'exact-time').textContent = `Good news! Unlimited Time Bypass! You are now qualified to get your account. Enjoy staying on this website!`;
                    }
    ...
    async function pullAccountCurrentStock(ver, name) {
        let ver_data = ver.split('-');
        let lang = ver_data.shift();
        let version = ver_data.join('-');

        let { data, error } = await supabase.rpc('get_specific_acc_qty', {
            p_name: name,
            p_ver: version,
            p_lang: lang
        });
    ...
    async function processAccount(account_name, account_id, account_ads, cart_fbdb_id, cart_data, lang_version) {
        ...
        let code = await reserveAccount(account_id, lang_version);

        if (!code) {
            window.alert(`Account with ID: ${account_id} and fields ${lang_version} are already depleted. Please ask for replenishment to Official Request Page or on Discord.`);
            return;
        }
        ...
        if (cart_data.status == "processing")
            ...
            await FirebaseModule.patch(
                `${db}/${btoa(user_email)}/${cart_fbdb_id}.json`,
                JSON.stringify({
                    heap_code: code
                }));
        }
        ...
        // UPDATED PARAMETER is SET FOR NEW UPDATE where all accounts are hosted in supabase.
        ...
        await supabase.rpc('increment_accreq_count', {
    ...
    async function getAccount(key, btn) {
        ...
        let lang = acc_ver.split('-').length === 3 ? "EN" : acc_ver.split('-')[0];
        ...
        processAccount(acc_data.name, acc_data.id, acc_data.ads, key, acc_data, spliceVersionData(acc_ver));
    }
    ...
    async function removeCartItem(user_email, key) {
        await FirebaseModule.patch(`${db}/${btoa(user_email)}/${key}.json`, 'null');
        decrementCartCount();
    }
    ...
    function spliceVersionData(str_version) {
        if (!str_version)
            return;

        // input is EN-15-4-1
        let splice = str_version.split('-');
        let lang = splice.shift();
        let version = splice.join('-');
    ...
    async function reserveAccount(account_id, acc_ver) {
        let { data, error } = await supabase.rpc('reserve_account_bucket', {
            account_id: account_id,
            p_lang: acc_ver.lang,
            p_ver: acc_ver.ver
        });
