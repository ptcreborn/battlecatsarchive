        ...
        keys.forEach(key => {
            ...
            parent.innerHTML += `<div style="                    
                    ...
                    "><div><span style="
                        ...
                        padding: 0 5px;
                        font-weight: 550;
                    ">ADMIN: ${item.email}</span> has added to accounts...<br><p style='line-height: 1.2rem;font-size: 0.9rem;'>New uploads: <b>[${item.acc} v${item.ver}]</b> — has now <b>${item.qty} account${item.stock > 1 ? `s` : ``} more!</b></p></div><span style="margin-left: auto; padding: 0 5px; font-weight: 550; line-height: 1rem; font-size: 12px; opacity: 0.7;">${moment(parseInt(key)).fromNow()}</span></div>`;
        });
    }
