window.addEventListener('load', async() => {
	...
	async function checkUrlInSPDB() {
		...
		let { data, error } = await supabase.from('bca-website-posts').select('id').eq('url', url);
		if (error) {
		...
		if (data.length == 0)
			return;
		const id = data[0].id;
		let comments_data = await supabase.from('bca-comments').select('fb_id, user_id(id, email, created_at, gender, prof_img, username, rank_id(rank_name, rank_image))').eq('bca_posts', id).order('date', { ascending: true });
