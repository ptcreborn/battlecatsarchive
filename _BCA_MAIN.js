        ...
        if (user_email) {
            try {
                ...
                new Promise(async (resolve, reject) => {
                    ...
                    let num_of_notifs = await Notifications.getNumberOfUnread();
...
(async () => {
    ...
    // For account buttons
    (async () => {
        const postBody = document.querySelectorAll('#postBody a');
        postBody.forEach(item => {
            let code = item.textContent.split('#');
            if (code.length !== 3)
                return;

            item.textContent = code[1].replaceAll('text=', '').replace('(', '').replace(')', '');
            item.className = 'button-15';
        });
    })();
