(async () => {
    ...
    async function addToCart(account_name, ads, id) {
        ...
        status_elem.innerHTML = `You have added <b><span add-qty-status="">${count_elem + 1}</span></b> in your cart. <a style='font-weight: bold;' target="_blank" href="https://battlecatsarchive.blogspot.com/p/cart.html?user=${btoa(userEmail)}">Checkout now ➡️</a>`;
    }
    ...
    async function listenLang() {
        ...
        versions.forEach(item => version_selector.innerHTML += `<option value="${item}">${item}</option>`);
    ...
    async function buildCompactMenu() {
        ...
        let supabase_data = await fetchAccountInfo();
        const version = version_selector.value;
        ...
        for (const item of supabase_data) {
            ...
            new Promise(async(resolve, reject) => {
                ...
                queryP(clone, 'store-account-info').querySelectorAll('p')[0].textContent = `${item.description.replaceAll('\n', ', ')}`; // cat food

                // queryP(clone, 'store-account-info').querySelectorAll('p')[1].textContent = `${item.description.split('\n')[1]}`; // XP

                // LISTENERS
                queryP(clone, 'store-account-request').id = `${item.id}`;
            ...
            new Promise(async (resolve, reject) => {
                let top_5_users = await topUserRequester(item.id);
